  ...
  <div id="table_container">
    ...
    <Row class="number_data">
      ...
      <div class="number_content">
        <div id="number_channel_chart5" :style="{width: '100%', height: '400px'}"></div>
      </div>
    </Row>
    ...
    <Row class="number_data">
      ...
      <div class="number_content">
        <i-Table :columns="tableData.qrcord.columns" :data="tableData.qrcord.pageList" @on-sort-change="handleQrcordSort"></i-Table>
        <div class="table_page">
          <div class="table_page_l">
            <p>共
              <span>{{tableData.qrcord.list.length}}</span> 条数据</p>
          </div>
          <div class="table_page_r">
            <Page :total="tableData.qrcord.list.length" :current.sync="tableData.qrcord.curPage" :page-size="10" show-elevator @on-change="changeQrcordPage" />
          </div>
        </div>
      </div>
    </Row>
...
export default {
  ...
  methods: {
    ...
    handleActivitySort(column) {
      if (this.tableData.activity.list.every(item => item[column.key] === 0)) return;
    ...
    setLineOptions() {
      ...
      options.xAxis.data =
        this.chartData.activity.length > 0
          ? this.chartData.activity.map(item => item.stat_ymd)
          : [];
    ...
    formatTable(type, data) {
      ...
      this.tableData[type].columns = new Array(
        this.tableData[type].columns.shift()
      );
    ...
    async getData(param) {
      ...
      this.tableData.qrcord.pageList =
        qrcord.length > 0 ? qrcord.slice(0, 10) : [];
  ...
  async created() {
    ...
    await this.$api.getActivityList({ market_id: this.market_id }).then(res => {
